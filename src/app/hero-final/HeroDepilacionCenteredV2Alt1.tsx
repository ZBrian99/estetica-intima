...
export default function HeroDepilacionCenteredV2Alt1() {
  return (
    <section className="relative min-h-[calc(100svh-4rem)] inset-0 flex flex-col justify-center items-center text-center px-6 text-white">
      ...
      <Image
        src="https://image.pollinations.ai/prompt/full%20body%20man%20and%20woman%20together%20photographic%20pose%20hairless%20smooth%20skin%20studio%20emerald%20teal%20cyan%20tones%20fashion%20aesthetic?width=1400&height=900&model=flux-realism&nologo=true&seed=8801"
        alt="Depilación hombres y mujeres"
        fill
        className="object-cover opacity-18 -z-10"
        priority
      />

      <h2 className="text-[42px] md:text-6xl font-bold leading-tight font-outfit">Depilación Definitiva</h2>
      ...
      <p className="mt-4 md:mt-6 max-w-3xl text-base md:text-xl text-white/90">
        Piel suave todo el año, sin irritación ni rutinas tediosas. Resultados visibles desde la primera sesión.
      </p>
      ...
      <div className="mt-10 flex flex-col xs:flex-row items-center justify-center gap-4">
        ...
        <Link href="/#contacto" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full border border-white text-white font-semibold hover:bg-white/10">
          <FaWhatsapp className="w-5 h-5" aria-hidden /> Escribinos
        </Link>
      </div>
    </section>
  )
