...
function PaginationButton({ onClick, disabled, title, icon, className }: PaginationButtonProps) {
	return (
		<button
			disabled={disabled}
			onClick={onClick}
			className={cn(
				'flex  h-10 w-8 us:min-w-10 items-center justify-center rounded-md text-base font-medium ring-offset-background transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 cursor-pointer touch-manipulation ',
				className
			)}
			title={title}
		>
	...
	return (
		<Link
			...
			className={cn(
				...
				'border transition-colors duration-200 font-medium  h-10 w-8 us:min-w-10 text-sm ',
				className
			)}
			{...props}
		/>
...
function PaginationEllipsis({ className, ...props }: React.ComponentProps<'span'>) {
	return (
		<span
			...
			className={cn('flex size-9 items-center justify-center text-gray-500', className)}
			{...props}
		>
